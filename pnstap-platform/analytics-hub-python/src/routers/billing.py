...
def _subscription_payload(row):
    ...
    plan = row["plan"]; config = PLANS.get(plan, PLANS["Free"])
...
@router.get("/plans")
def get_plans():
    return {"trial":{"days":14,"access":"full platform access"},"overage":{"price":10,"unit":"user/month","applies_to":"users above plan limit"},"plans":PLANS,"payment_methods":["Stripe","PayPal","Bank transfer (Enterprise)"]}
