...
PLANS={"Free":{"annual_price":0,"user_limit":1,"retention_days":7,"ai":"Core"},"Pro":{"annual_price":1000,"user_limit":5,"retention_days":30,"ai":"Basic"},"Business":{"annual_price":5000,"user_limit":50,"retention_days":90,"ai":"Advanced"},"Enterprise":{"annual_price":15000,"user_limit":1000000,"retention_days":365,"ai":"Custom"}}
class PlanSelection(BaseModel): plan:str=Field(...,min_length=2,max_length=30);billing_cycle:str=Field(default="Yearly",max_length=20);payment_provider:str=Field(default="stripe",max_length=30)
def _subscription_payload(row):
 now=datetime.now(timezone.utc);trial_ends=row["trial_ends_at"]
 if trial_ends.tzinfo is None:trial_ends=trial_ends.replace(tzinfo=timezone.utc)
 trial_active=row["status"]=="TRIALING" and trial_ends>now;days_remaining=max(0,(trial_ends-now).days) if trial_active else 0;plan=row["plan"];config=PLANS.get(plan,PLANS["Free"]);access=row["status"] in {"ACTIVE","TRIALING"} and (row["status"]=="ACTIVE" or trial_active)
...
@router.get("/plans")
def get_plans(): return {"trial":{"days":7,"access":"full platform access"},"overage":{"price":10,"unit":"user/month","applies_to":"users above plan limit"},"plans":PLANS,"payment_methods":["Stripe","PayPal","Bank transfer (Enterprise)"]}
...
def get_subscription(current_user:dict=Depends(get_current_user)):
 ...
 try:
  with connection.cursor() as cursor:
   cursor.execute("SELECT * FROM subscriptions WHERE company_id = %s",(current_user["company_id"],));row=cursor.fetchone()
